...
5. 扫描性能与跳过报告
   ...
   扫描必须避免因为超大文件一次性读入内存而卡住，同时要把无法完整扫描的文件明确报告出来。

   - [x] 小文件完整读入并执行完整扫描
   - [x] 大文件不整体读入内存
   - [x] 大文件按块读取，每次读取几 MB
   - [x] 大文件块内只执行高置信密钥模式扫描
   - [x] 大文件块扫描保留适当 overlap，避免跨块密钥漏检
   - [x] 超过极大阈值的文件直接跳过
   - [x] 为跳过文件输出结构化 skipped 结果
   - [x] `scan` 文本输出报告 skipped 文件
   - [x] `scan --json` 报告 skipped 文件
   - [x] `audit` 报告 skipped 文件
   - [x] Python API 能获取 skipped 信息
   - [x] 为大文件分块扫描、跨块匹配和 skipped 报告补测试
